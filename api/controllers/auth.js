...
export const signUp = async (req, res) => {
  ...
  if(error) return res.status(501).json({message: `Validation error: ${error.details[0].message}`, success: false});

  const {password, ...others} = req.body;
  //Check for password length
  if(password.length < 6) return res.status(401).json({message: `Password must be greater than 5`, success: false});
